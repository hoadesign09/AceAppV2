...
const Geolocation = {
  setRNConfiguration: (config: any) => {}, // eslint-disable-line no-unused-vars

  requestAuthorization: async (authorizationLevel: any) => {
    if (Platform.OS !== 'ios') {
  ...
  getCurrentPosition: (success: any, error = noop, options = {}) => {
    if (!success) {
      // eslint-disable-next-line no-console
  ...
  watchPosition: (success: any, error = null, options = {}) => {
    if (!success) {
      // eslint-disable-next-line no-console
      console.error('Must provide a success callback');
    }
